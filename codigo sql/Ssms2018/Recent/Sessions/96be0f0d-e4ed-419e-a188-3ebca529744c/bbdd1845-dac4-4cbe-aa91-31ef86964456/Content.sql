...
DECLARE  @PARAMETROS NVARCHAR(MAX)			,@MODELO VARCHAR(100)			   ,@METODO VARCHAR(100)
      ...
      ,@NOMBREXML VARCHAR(120)        ,@XML VARCHAR(MAX)            ,@NOMBREJSON VARCHAR(120)      ,@RIPSJSON VARCHAR(MAX)
   BEGIN
	SELECT *
	INTO #JSON
   ...
   BEGIN
      ...
      WITH   (
       ...
       PRINT 'Capturo los datos de la factura'
       SELECT @IDDEP=IDDEP,@INDCARTERA=INDCARTERA,@INDCXC=INDCXC,@PROCEDENCIA=PROCEDENCIA,@IDSEDE=IDSEDE,@CAPITA=COALESCE(CAPITADA,0),
              @NOREFERENCIA=NOREFERENCIA,@TIPOFACT=TIPOFAC,@CONTABILIZADA=CONTABILIZADA
       FROM FTR WHERE N_FACTURA=@N_FACTURA
       IF @IDDEP<> DBO.FNK_VALORVARIABLE('IDFDEPFACTURACION')
       BEGIN
			INSERT INTO @TBLERRORES(ERROR)
      ...
      END
      IF @PROCEDENCIA='CI' AND @NOREFERENCIA<>'VARIOS' AND NOT EXISTS(SELECT * FROM CIT WHERE CONSECUTIVO=@NOREFERENCIA) 
      BEGIN
			INSERT INTO @TBLERRORES(ERROR)
      ...
      BEGIN
         ...
         BEGIN TRY
		      ...
		      INSERT INTO FTR(CNSFCT, COMPANIA, CLASE, IDTERCERO, N_FACTURA, F_FACTURA, F_VENCE, VR_TOTAL, COBRADOR, VENDEDOR, MONEDA, OCOMPRA, ESTADO, F_CANCELADO,
						      ...
						      IDSEDE, RDIAN, CNSRESOL,IDFORMATO,COPAPROPIO,CP_VLR_COPAGOS,CAPITADA,CP_CONVENIO,CP_MODALIDAD,CP_MES,FECHACAP_INI,FECHACAP_FIN)
		      SELECT @CNSFTR, @COMPANIA, 'C', @IDTERCEROF, @NVOCONSEC, CASE  WHEN  @F_FACTURA IS NOT NULL THEN @F_FACTURA ELSE DBO.FNK_GETDATE() END,
                  CASE  WHEN  @F_FACTURA IS NOT NULL THEN @F_FACTURA+ @DV ELSE DBO.FNK_GETDATE()+ @DV END,0, NULL, NULL, LEFT(@DATO,2), NULL, 'P', NULL, 
				      NULL, @USUARIO, NULL, 'FINANCIERO', 'I', REPLACE(COALESCE(@OBSERVACION,''),'@N_FACTURA',@NVOCONSEC), @TV, 0, 0, 0, 0, 
               ...
               @VLRCOPAPRO,@CAPIPGP,@CP_CONVENIO,@CP_MODALIDAD,@CP_MES,@FECHACAP_INI,@FECHCACA_FIN
   ...
   BEGIN
      ...
      @NOADMISION=NOADMISION,@PROCESO=PROCESO
      FROM   OPENJSON (@DATOS)
      WITH   (
               CNSFTR  VARCHAR(20)    '$.CNSFTR',
               N_CUOTA SMALLINT       '$.N_CUOTA',
      ...
      BEGIN
         BEGIN TRY 
            IF @PROCESO='ADD'
            BEGIN
               EXEC DBO.SPK_ADDSERCAP @CNSFTR,@N_CUOTA,@IDTERCERO,@IDPLAN,@FECHACAP_INI,@FECHCACA_FIN,@PROCEDENCIA,@CLASEORDEN,@IDPESPECIAL,@RIESGO,@NOADMISION
            END
            IF @PROCESO='DEL'
            BEGIN
               PRINT 'EXEC DBO.SPK_DELSERCAP '+@CNSFTR+','+STR(@N_CUOTA)+','+@IDTERCERO+','+@IDPLAN+','+CONVERT(VARCHAR,@FECHACAP_INI,103)+','+CONVERT(VARCHAR,@FECHCACA_FIN,103)+','+@PROCEDENCIA+','+@NOADMISION
               EXEC DBO.SPK_DELSERCAP @CNSFTR,@N_CUOTA,@IDTERCERO,@IDPLAN,@FECHACAP_INI,@FECHCACA_FIN,@PROCEDENCIA,@NOADMISION
            END
         END TRY 
         BEGIN CATCH
            INSERT INTO @TBLERRORES(ERROR) SELECT ERROR_MESSAGE()
   ...
   BEGIN 
      ...
      BEGIN
         IF @PROCEDENCIA='CIT' OR  @PROCEDENCIA='CITCE'
         BEGIN
            PRINT 'V0Y A FACTURAR '+COALESCE(@IDSEDEF,'SIN SEDE FILTRO')
            BEGIN TRY 
               EXEC SPK_FACTURACI_ESCMA @CNSFACT,@NIT,@COMPANIA,@IDSEDE,@USUARIO,@IDTERCERO,@IDPLAN,'',@IDTERCERO,@F_FACTURA,NULL,@IDSEDEF
            END TRY 
            BEGIN CATCH
               INSERT INTO @TBLERRORES(ERROR) SELECT ERROR_MESSAGE()
            END CATCH
   ...
   BEGIN
      ...
      PRINT @FECHAINI
      PRINT @FECHAFIN
      SELECT FTR.N_FACTURA,VK.TIPODOC
      FROM FTR INNER JOIN VWK_ESTADODIAN_FTRE VK ON FTR.CNSFCT=VK.CNSFCT
