...
DECLARE	 @PARAMETROS   NVARCHAR(MAX),@MODELO           VARCHAR(100) ,@METODO    VARCHAR(100)	,@USUARIO      VARCHAR(12)
		,@GRUPO        VARCHAR(8)	,@SYS_COMPUTERNAME VARCHAR(254) ,@SEDE      VARCHAR(5)		,@FECHA        VARCHAR(20)   
		...
		,@FACTURADA    BIT          ,@N_FACTURA         VARCHAR(20)	 ,@HABILITA_COPAGO BIT      ,@CODIGO       VARCHAR(20)
...
DECLARE @TIPOCITA_ANT VARCHAR(20), @MODALIDAD_ANT VARCHAR(20), @TIPOCITA_NUE VARCHAR(20), @MODALIDAD_NUE VARCHAR(20)	,@SERVICIO_2 NVARCHAR(MAX)
		...
		, @IDAUTSES INT	,@IDMEDICO_1 VARCHAR(20)	,@IDMEDICO_2 VARCHAR(20), @TIPOFAC VARCHAR(20)	,@DPR VARCHAR(20)		,@VIVA DECIMAL(14,2)
DECLARE @AM BIT
		DECLARE @VIRTUAL BIT
...
BEGIN
	...
	BEGIN
		...
		SELECT @FECHA = DIA, @IDEMEDICA = ESPECIALIDAD ,@IDMEDICO = IDMEDICO
			,@MODALIDAD = MODALIDAD, @IDSEDE = IDSEDE, @DISPONIBILIDAD = DISPONIBILIDAD, @IDTERCERO = IDTERCERO
		FROM OPENJSON(@PARAMETROS)
		WITH(DIA                VARCHAR(20)       '$.DIA'
      ...
      , (SELECT COUNT(1) FROM CIT C WHERE CIT.IDMEDICO = C.IDMEDICO AND CIT.IDEMEDICA = C.IDEMEDICA AND CONVERT(DATE, CIT.FECHA) = CONVERT(DATE,C.FECHA) AND C.IDAFILIADO IS NOT NULL AND COALESCE(C.ESPROGRAMAS,0) = 0 ) [OCUPADO]
      , (SELECT COUNT(1) FROM CIT C WHERE CIT.IDMEDICO = C.IDMEDICO AND CIT.IDEMEDICA = C.IDEMEDICA AND CONVERT(DATE, CIT.FECHA) = CONVERT(DATE,C.FECHA) AND C.IDAFILIADO IS NULL AND COALESCE(C.ESPROGRAMAS,0) = 0 ) [DISPONIBLE]
      , (SELECT COUNT(1) FROM CIT C WHERE CIT.IDMEDICO = C.IDMEDICO AND CIT.IDEMEDICA = C.IDEMEDICA AND CONVERT(DATE, CIT.FECHA) = CONVERT(DATE,C.FECHA) AND COALESCE(C.ESPROGRAMAS,0) = 0 ) [TOTAL]
		FROM   CIT INNER JOIN MED ON CIT.IDMEDICO   = MED.IDMEDICO
		            LEFT JOIN AFI ON CIT.IDAFILIADO = AFI.IDAFILIADO
		WHERE  CIT.IDEMEDICA = @IDEMEDICA AND MED.TIPO_USUARIO <> 'Silla' AND COALESCE(CIT.ESPROGRAMAS,0) = 0
		...
		AND CIT.IDSEDE = CASE WHEN COALESCE(@IDSEDE,'') <> '' THEN @IDSEDE ELSE CIT.IDSEDE END
		GROUP BY CIT.IDMEDICO, CIT.IDEMEDICA, MED.IDMEDICO, MED.NOMBRE, CONVERT(DATE, CIT.FECHA)
		--CONSECUTIVO CONCAT AFI IDPLANPART2 //ESTADO//IDSEDE PART EMAIL
		SELECT @SQL ='
		SELECT  CIT.FECHA, NAME=MED.IDMEDICO, VALUE=CONVERT(VARCHAR(8000)
			...
			WHEN COALESCE(CIT.CUMPLIDA,0) = 2  THEN ''NO ASISTIO''
            ...
            ,  CIT.TIPOCITA [TIPOCITA], T3.DESCRIPCION [TIPOCITA_DESC]
                ...
                , (SELECT LEFT(STUFF(( SELECT CHAR(10) + MPE.IDPESPECIAL+ '' - '' + MPE.DESCESPECIAL FROM DBO.FNK_SPLIT(( SELECT MODELOS FROM CIT C WHERE C.CONSECUTIVO = CIT.CONSECUTIVO),'','',0,NULL) A  INNER JOIN MPE ON MPE.IDPESPECIAL=A.ITEM FOR XML PATH('''')),1,0,''''),1040) ) [NOMBREMODELO] FOR JSON PATH ) 
                        ELSE (SELECT ''OCUPADO'' ESTADO, CIT.IDSEDE
            ,CASE 
				WHEN (SELECT COUNT(1) FROM CIT C INNER JOIN MAGPRP ON C.IDMEDICO = MAGPRP.IDMEDICO AND C.FECHA  >= MAGPRP.FECHAINICIAL AND C.FECHA< MAGPRP.FECHAFINAL WHERE  C.CONSECUTIVO = CIT.CONSECUTIVO) >0 THEN ''BLOQUEO''
				WHEN COALESCE(CIT.CONFIRMADA,0) = 0 AND CIT.FECHALLEGA IS NULL AND CIT.USUARIOLLEGA IS   NULL AND CIT.FECHASOL IS NOT NULL THEN ''OCUPADO''
				WHEN COALESCE(CIT.CUMPLIDA,0) = 2  THEN ''NO ASISTIO''
            ...
            ,MPE.IDPESPECIAL, MPE.DESCESPECIAL, COALESCE(CIT.VALORCOPAGO,0) [COPAGO] 
                ...
                THEN  CASE  WHEN CIT.TIPOCAJA = ''TFCJ'' THEN  ((COALESCE(CIT.VALORTOTAL,0))-(COALESCE(CASE WHEN  TIPODTO = ''V'' THEN CIT.DESCUENTO WHEN TIPODTO=''P'' THEN (CIT.DESCUENTO * CIT.VALORTOTAL) / 100 END,0))) 
                            WHEN CIT.TIPOCAJA = ''FCJ'' THEN  ((COALESCE(CIT.VALORTOTAL,0))-(COALESCE(CASE WHEN  TIPODTO = ''V'' THEN CIT.DESCUENTO WHEN TIPODTO=''P'' THEN (CIT.DESCUENTO * CIT.VALORTOTAL) / 100 END,0)))  END 
							ELSE (CIT.VALORCOPAGO-COALESCE(CASE WHEN  TIPODTO = ''V'' THEN CIT.DESCUENTO WHEN TIPODTO=''P'' THEN (CIT.DESCUENTO * CIT.VALORTOTAL) / 100 END,0)) END [VALORCOPAGO]
			, MSGPAGO =  CASE WHEN DBO.FNK_VALORVARIABLE(''IXCOUNTRY'') = ''PERU''  AND (EXISTS (SELECT  * FROM PCJ WHERE PCJ.CODCAJA = CIT.CODCAJA AND PCJ.CNSFACJ = CIT.NORECIBOCAJA) 
				...
				WHEN DBO.FNK_VALORVARIABLE(''IXCOUNTRY'') <> ''PERU'' AND COALESCE(CIT.GENEROCAJA, 0) = 0 THEN ''Pago'' ELSE ''Sin Pagar'' END 
			, COALESCE(CIT.IDKPAGE,0) IDKPAGE, COALESCE(KPAGE.LINKPAGO,'''') LINKPAGO, KPAGE.ESTADO [KPAGE_ESTADO], KPAGE.F_EXPIRA, COALESCE(AFI.EMAIL,'''') EMAIL
            ,CASE WHEN CIT.GENEROCAJA <> 1  THEN ''''
                WHEN CIT.TIPOCAJA = ''TFCJ''  AND (SELECT TOP 1 FCJ.ESTADO  FROM FCJ WHERE  FCJ.NOADMISION = CIT.CONSECUTIVO AND FCJ.PROCEDENCIA IN (''CIT'', ''CITAS'' )) <> ''A'' THEN '' - Recibo sin enviar a caja''
                WHEN CIT.TIPOCAJA = ''FCJ''   AND (SELECT TOP 1 FCJ.CERRADA FROM FCJ WHERE  FCJ.NOADMISION = CIT.CONSECUTIVO AND FCJ.PROCEDENCIA IN (''CIT'', ''CITAS'' )) = 0 AND (SELECT TOP 1 FCJ.ESTADO FROM FCJ WHERE  FCJ.NOADMISION = CIT.CONSECUTIVO AND FCJ.PROCEDENCIA IN (''CIT'', ''CITAS'' ))<> ''A'' THEN '' - Recibo sin recaudar''
                WHEN CIT.TIPOCAJA = ''FCJ''   AND (SELECT TOP 1 FCJ.ESTADO  FROM FCJ WHERE  FCJ.NOADMISION = CIT.CONSECUTIVO AND FCJ.PROCEDENCIA IN (''CIT'', ''CITAS'' )) = ''A'' THEN '' - Recibo anulado''
                WHEN CIT.TIPOCAJA = ''TFCJ''  AND (SELECT TOP 1 FCJ.ESTADO  FROM FCJ WHERE  FCJ.NOADMISION = CIT.CONSECUTIVO AND FCJ.PROCEDENCIA IN (''CIT'', ''CITAS'' )) = ''A'' THEN ''''
                WHEN CIT.TIPOCAJA = ''FCJ''   AND (SELECT TOP 1 FCJ.CERRADA FROM FCJ WHERE  FCJ.NOADMISION = CIT.CONSECUTIVO AND FCJ.PROCEDENCIA IN (''CIT'', ''CITAS'' )) = 1 AND (SELECT TOP 1 FCJ.ESTADO FROM FCJ WHERE  FCJ.NOADMISION = CIT.CONSECUTIVO AND FCJ.PROCEDENCIA IN (''CIT'', ''CITAS'' ))<> ''A'' THEN '' - Recibo recaudado''
                ...
                END [ESTADO_REGISTRO]
			, COALESCE(CIT.NFACTURA,'''') NFACTURA, COALESCE(N_FACTURA,'''') N_FACTURA, COALESCE(CIT.URGENCIA,0) URGENCIA
			...
			, CASE WHEN EXISTS (SELECT * FROM CITSITEDS WHERE CITSITEDS.CONSECUTIVO = CIT.CONSECUTIVO) THEN ''SI'' ELSE ''NO'' END [ESTADO_SITEDS]
			, CIT.USUARIO, USUSU_ASIGNA.NOMBRE [NOMBRE_ASIGNA],CIT.FECHALLEGA, CIT.USUARIOLLEGA,USUSU_LLEGA.NOMBRE [NOMBRE_LLEGA] ,CIT.FECHAATENCION
																FROM AFI
																...
																LEFT JOIN KPAGE ON CIT.IDKPAGE = KPAGE.IDKPAGE AND CIT.CONSECUTIVO = KPAGE.CONSECUTIVO AND KPAGE.TABLA = ''CIT''
																...
																LEFT JOIN USUSU USUSU_LLEGA ON CIT.USUARIOLLEGA = USUSU_LLEGA.USUARIO
																LEFT JOIN USUSU USUSU_ASIGNA ON CIT.USUARIO = USUSU_ASIGNA.USUARIO
																WHERE AFI.IDAFILIADO = CIT.IDAFILIADO  AND COALESCE(CIT.ESPROGRAMAS,0) = 0
																FOR JSON PATH )
														END
														))
		FROM   CIT INNER JOIN MED ON CIT.IDMEDICO = MED.IDMEDICO
		LEFT JOIN TGEN T3 ON CIT.TIPOCITA = T3.CODIGO AND T3.TABLA = ''CITAS'' AND T3.CAMPO = ''TIPOCITA''
		WHERE COALESCE(CIT.ESPROGRAMAS,0) = 0 AND  CIT.IDEMEDICA = '''+@IDEMEDICA+'''  AND CONVERT(DATE, CIT.FECHA) = CONVERT(DATE, '''+CONVERT(VARCHAR,@FECHA1,105)+''')
		'
		--SELECT TOP 100  USUARIO,FECHALLEGA, USUARIOLLEGA,FECHAATENCION, * FROM  CIT WHERE  CONSECUTIVO = '010023036988'

		--IDTERCEROPARTICULAR--SITEDS
		--VALORCOPAGO--VALORTOTAL
		...
		BEGIN
			SELECT @SQL += ' AND  CIT.IDTERCEROCA = '''+@IDTERCERO+''''
		END
		IF COALESCE(@IDTERCERO,'') = ''
		BEGIN
			SELECT @SQL +=  ' AND  1 = 1'
	...
	BEGIN
      ...
      BEGIN TRY
         IF @TIPODESCUENTO = 0 AND DBO.FNK_VALORVARIABLE('IXCOUNTRY') <> 'PERU'
         BEGIN
            UPDATE CIT SET DESCUENTO = @VALORDESCUENTO, TIPODTO = 'V' WHERE  CONSECUTIVO = @CONSECUTIVO
         END
         IF @TIPODESCUENTO = 1 AND DBO.FNK_VALORVARIABLE('IXCOUNTRY') <> 'PERU'
         BEGIN
		 ...
		 BEGIN
			UPDATE CIT SET VALORTOTAL = @VALORDESCUENTO WHERE CONSECUTIVO = @CONSECUTIVO
		 END
		END TRY
      BEGIN CATCH
	...
	BEGIN 
		...
		BEGIN
			...
			BEGIN
				...
				SELECT @VALORTOTAL = (SELECT CIT.VALORTOTAL+ @VIVA FROM  CIT WHERE CONSECUTIVO = @CONSECUTIVO)
		...
		SELECT @CNSACJ =  CAJ.CNSACJ, @CODCAJERO = USUSU.CODCAJERO, @CODCAJA = UBEQ.CAJA  FROM  USUSU
					...
					INNER JOIN CAJ ON UBEQ.CAJA = CAJ.CODCAJA
				WHERE  USUARIO = @USUARIO
		SELECT -- CNSFACJ CODCAJA TOTALFCJ
...
-- PARA EL ACCIDENTE DE TRANSITO	
				...
				ELSE
					BEGIN
						...
						BEGIN
							...
							SELECT @ITEM = MAX(ITEM)+1 FROM HACTRAND WHERE  CNSHACTRAN	=@CNSHACTRAN 
		...
		FROM CIT WHERE  CONSECUTIVO = @CONSECUTIVO
			...
			BEGIN
				UPDATE CIT SET CIT.VALORCOPAGO = CITSITEDS.COPAGOCONSULTA, CIT.COPA_VAR_PORC = CITSITEDS.COPAGOVARIABLECONSULTA FROM CIT,CITSITEDS WHERE CIT.CONSECUTIVO = @CONSECUTIVO AND CIT.CONSECUTIVO = CITSITEDS.CONSECUTIVO  
				RETURN
			END
			IF EXISTS (SELECT * FROM #AFILIADOSITEDS WHERE COALESCE(COPAGOCONSULTA,'')<> '')
			BEGIN
				UPDATE CIT SET CIT.VALORCOPAGO = #AFILIADOSITEDS.COPAGOCONSULTA, CIT.COPA_VAR_PORC = #AFILIADOSITEDS.COPAGOVARIABLECONSULTA FROM CIT,#AFILIADOSITEDS WHERE CIT.CONSECUTIVO = @CONSECUTIVO
				UPDATE TFCJ SET TFCJ.VALORTOTAL = CIT.VALORCOPAGO FROM TFCJ, CIT WHERE TFCJ.NOADMISION = @CONSECUTIVO AND CIT.CONSECUTIVO = @CONSECUTIVO AND TFCJ.PROCEDENCIA = 'CITAS' 
	...
	BEGIN
	   ...
	   WITH (CONSECUTIVO       VARCHAR(20)    '$.CONSECUTIVO',
		...
		SELECT @VLR_ANTERIOR = VALORCOPAGO,@IDTERCEROCA = IDTERCEROCA, @IDAFILIADO = IDAFILIADO, @SEDE = IDSEDE FROM CIT WHERE  CONSECUTIVO = @CONSECUTIVO
      ...
      PRINT @SEDE
		IF  (SELECT DBO.FNK_VALORVARIABLE('QCITAVALIDAFOTO')) = 'SI' 
			AND EXISTS (SELECT * FROM AFI WHERE  IDAFILIADO = @IDAFILIADO AND COALESCE( LTRIM(RTRIM(AFI.IDFOTO)),'') = '')
         AND EXISTS (SELECT * FROM SED WHERE SED.IDSEDE = @IDSEDE AND EXTRAMURAL = 0)
		BEGIN
         INSERT INTO @TBLERRORES(ERROR) SELECT 'No puede marcar llegada, el paciente requiere una Foto en su registro, favor de Editar los datos del paciente y registrar una foto.'
			...
			RETURN
		END
		IF  EXISTS (SELECT * FROM CIT WHERE  CONSECUTIVO = @CONSECUTIVO AND COALESCE(CIT.CUMPLIDA,0) = 2)
		BEGIN
			INSERT INTO @TBLERRORES(ERROR) SELECT 'No puede marcar llegada, La cita se encuentra como "NO ASISTIO", favor de quitarlo de ese estado.'
			SELECT 'KO' OK
			SELECT ERROR FROM @TBLERRORES
			RETURN
		END
    ...
    BEGIN
      ...
      SELECT @CNSACJ =  CAJ.CNSACJ, @CODCAJERO = USUSU.CODCAJERO, @CODCAJA = UBEQ.CAJA  FROM  USUSU
        ...
        BEGIN
            IF  (SELECT top 1 TUR.LUNES  FROM     CXT INNER JOIN TUR ON CXT.CODTURNO = TUR.CODTURNO WHERE  CODCAJA = @CODCAJA AND CODCAJERO =@CODCAJERO) IN (NULL, '', 0)
            BEGIN
        ...
        BEGIN
            IF  (SELECT top 1 TUR.MARTES  FROM     CXT INNER JOIN TUR ON CXT.CODTURNO = TUR.CODTURNO WHERE  CODCAJA = @CODCAJA AND CODCAJERO =@CODCAJERO) IN (NULL, '', 0) 
            BEGIN
        ...
        BEGIN
            IF  (SELECT top 1 TUR.MIERCOLES  FROM     CXT INNER JOIN TUR ON CXT.CODTURNO = TUR.CODTURNO WHERE  CODCAJA = @CODCAJA AND CODCAJERO =@CODCAJERO) IN (NULL, '', 0) 
            BEGIN
        ...
        BEGIN
            IF  (SELECT top 1 TUR.JUEVES  FROM     CXT INNER JOIN TUR ON CXT.CODTURNO = TUR.CODTURNO WHERE  CODCAJA = @CODCAJA AND CODCAJERO =@CODCAJERO)IN (NULL, '', 0) 
            BEGIN
        ...
        BEGIN
            IF  (SELECT top 1 TUR.VIERNES  FROM     CXT INNER JOIN TUR ON CXT.CODTURNO = TUR.CODTURNO WHERE  CODCAJA = @CODCAJA AND CODCAJERO =@CODCAJERO) IN (NULL, '', 0) 
            BEGIN
        ...
        BEGIN
            IF  (SELECT top 1 TUR.SABADO  FROM     CXT INNER JOIN TUR ON CXT.CODTURNO = TUR.CODTURNO WHERE  CODCAJA = @CODCAJA AND CODCAJERO =@CODCAJERO)IN (NULL, '', 0)
            --IF  1= 1
            BEGIN
        ...
        BEGIN
            IF  (SELECT top 1  TUR.DOMINGO  FROM     CXT INNER JOIN TUR ON CXT.CODTURNO = TUR.CODTURNO WHERE  CODCAJA = @CODCAJA AND CODCAJERO =@CODCAJERO) IN (NULL, '', 0) 
            BEGIN
...
--query3
   IF @METODO = 'FACTURAR_CITA'
	BEGIN
	   SELECT @CONSECUTIVO=CONSECUTIVO, @IDTERCERO = IDTERCERO , @FECHAFACT = FECHAFACT, @TIPOFAC = TIPOFAC, @CNSFACJ = CNSFACJ, @CODCAJA = CODCAJA
	   FROM OPENJSON(@PARAMETROS)
	   WITH( CONSECUTIVO			VARCHAR(20)		'$.CONSECUTIVO'
			,IDTERCERO				VARCHAR(20)		'$.IDTERCERO'
			,FECHAFACT				DATE			'$.FECHAFACT'	
	  ...
	  --SELECT @TIPOFAC
		...
		SELECT @IDPLAN = IDPLAN, @IDTERCEROCA = IDTERCEROCA FROM CIT WHERE CONSECUTIVO = @CONSECUTIVO
		IF DBO.FNK_VALORVARIABLE('IXCOUNTRY')='PERU' AND 
		EXISTS(SELECT * FROM USVGS WHERE IDVARIABLE LIKE 'IDPLANPART%' AND DATO=@IDPLAN) AND (SELECT DBO.FNK_VALORVARIABLE('IDTERCEROPARTICULAR')) = @IDTERCEROCA
		BEGIN
		...
		END
      IF @CUMPLIDA <> 1 AND DBO.FNK_VALORVARIABLE('IXCOUNTRY')<> 'PERU'
      BEGIN
		...
		BEGIN
            ...
            BEGIN
			   ...
			   IF @TIPOFAC ='Boleta'
				...
				EXEC SPK_FACTURACE_PERU_COPA @CONSECUTIVO,@DPR,@COMPANIA, @SEDE, @USUARIO,'','','','','','CI','', 'FALSE', NULL,@FECHAFACT,@TIPOFAC, @IDTERCERO   
               --EXEC SPK_FACTURACE_PERU @CONSECUTIVO, @DPR,'01',@SEDE,@USUARIO,'', '', '', '', '','CI','','FALSE', NULL, @FECHAFACT,@TIPOFAC, @IDTERCERO

			   SELECT @N_FACTURA = N_FACTURA FROM CIT WHERE CONSECUTIVO = @CONSECUTIVO
			   UPDATE FCJ SET N_FACTURA = @N_FACTURA, RECAUDO = 1 WHERE FCJ.NOADMISION = @CONSECUTIVO
			   UPDATE FTRD SET FTRD.VR_TOTAL = FTR.VR_TOTAL , FTRD.VLR_SERVICI = FTR.VR_TOTAL 
			   ,FTRD.VALOR = ROUND(FTR.VR_TOTAL/1.18,2)  
			   , FTRD.VIVA =  FTR.VR_TOTAL -ROUND(FTR.VR_TOTAL/1.18,2)
			   FROM FTR,FTRD WHERE FTR.N_FACTURA = @N_FACTURA AND  FTRD.N_FACTURA = @N_FACTURA
            ...
            END
			   ...
			   BEGIN
			   PRINT 'ENTRA ACA, POR ULTIMA VEZZZ'
	...
	BEGIN
		...
		BEGIN TRY
			...
			SELECT @IDPLAN = IDPLAN, @IDTERCEROCA = IDADMINISTRADORA FROM AFI WHERE IDAFILIADO = @IDAFILIADO
			IF EXISTS(SELECT * FROM USVGS WHERE IDVARIABLE LIKE 'IDPLANPART%' AND DATO=@IDPLAN) AND (SELECT DBO.FNK_VALORVARIABLE('IDTERCEROPARTICULAR')) = @IDTERCEROCA
			BEGIN
				SELECT 1 PARTICULAR
			END 
			...
			IF EXISTS(SELECT * FROM CIT INNER JOIN SED ON CIT.IDSEDE = SED.IDSEDE WHERE COALESCE(SED.EXTRAMURAL,0) = 1 AND CIT.CONSECUTIVO = @CONSECUTIVO) 
				AND EXISTS (SELECT * FROM TGEN WHERE TABLA = 'AFI' AND CAMPO = 'TIPOATENCION' AND VALOR1 = 1)
			BEGIN
				SELECT TOP 1 CODIGO, DESCRIPCION FROM TGEN WHERE TABLA = 'AFI' AND CAMPO = 'TIPOATENCION' AND VALOR1 = 1
			END
