...
BEGIN  
   ...
   BEGIN
      ...
      IF @IXCOUNTRY='PERU'
      BEGIN
         SELECT @VLR_AFECTO=SUM(CASE WHEN COALESCE(PIVA,0)>0 THEN COALESCE(VLRIMPUESTO,0)  ELSE 0 END), 
         @VLR_INAFECTO=SUM(CASE WHEN COALESCE(PIVA,0)=0 THEN (VLR_SERVICI-COALESCE(VLR_COPAGOS,0)) ELSE 0 END),
         @VLR_SUBTOTAL=SUM(CASE WHEN  COALESCE(PIVA,0)>0 THEN VLR_SERVICI/(1+(PIVA/100)) ELSE VLR_SERVICI END)
         FROM FTRD 
