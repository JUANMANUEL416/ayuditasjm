...
DECLARE @RESIDUO INT 
DECLARE @POR_RESIDIO DECIMAL(14,2)
...
BEGIN
   ...
   BEGIN  
      ...
      BEGIN   
         ...
         BEGIN
            ...
            SELECT @DIASLABT=DBO.FNK_DATEDIFF360(@FECHAING,@FECHARET),@MESESLABA=DATEDIFF(MONTH,@ENERO,@FECHARET),
                   @FECHAING=FECHAING
   ...
   BEGIN
     ...
     -- RETURN
      ...
      SELECT NUMDOC,COALESCE(INDEMNIZA,0)FROM 
      NEDIA WHERE CNSPAGO = @CNSPAGO AND IDEVENTO = 'RET' 
      AND NUMDOC=CASE WHEN COALESCE(@NUMDOR,'')<>'' THEN @NUMDOR ELSE NUMDOC END
      OPEN RET_EMP       
      ...
      BEGIN   
         PRINT 'NUMDOC= '+@NUMDOC
         SELECT @FECHARET=FECHAFIN 
         FROM  NEDIA 
         WHERE CNSPAGO = @CNSPAGO 
         AND IDEVENTO = 'RET'
         AND NUMDOC=@NUMDOC

         SELECT  @DIASLABT=DBO.FNK_DATEDIFF360(@FECHAING,@FECHARET),@FECHAING=FECHAING,@CANTI=HORASXCARGO,@BASICOT=BASICO,
         @BASICO=CONVERT(decimal(14,2),(BASICO/DIASXCARGO)) ,@MESESLABA=DATEDIFF(MONTH,FECHAING,@FECHARET+1) 
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN 
                  ...
                  BEGIN
                     SELECT @DIASR=COALESCE(NDIAS,0) FROM NEMPVD WHERE NUMDOC=@NUMDOC AND ITEM=@ITEMVAC AND CNSPAGO=@CNSPAGO  AND EVENTO='RET'  
                     DELETE NEMPVD WHERE NUMDOC=@NUMDOC AND   CNSPAGO=@CNSPAGO AND ITEM=@ITEMVAC AND EVENTO='RET'
                     UPDATE NEMPV SET DIASP=DIASP-COALESCE(@DIASR,0) WHERE NUMDOC=@NUMDOC AND ITEM=@ITEMVAC
            ...
            BEGIN
               PRINT 'VOY A INDEMNIZAR @PERINDEM='+@PERINDEM
               SELECT @DIASLABT=DBO.FNK_DATEDIFF360(@FECHAING,@FECHARET)
               ...
               IF @PERINDEM='MENOR'
               BEGIN
                  PRINT 'VOY POR MENOR @DIASLABT='+CAST(@DIASLABT AS VARCHAR(20))
                  IF @DIASLABT < 360
                  BEGIN
               ...
               ELSE
               BEGIN
                  PRINT 'VOY POR MAYOR @DIASLABT='+CAST(@DIASLABT AS VARCHAR(20))

                  IF  COALESCE(@DIASLABT,0) < 360
                  BEGIN
                     PRINT 'DEBI INGRESAR ACA'
                     SELECT @VALORD = 0
                  END
                  ELSE
                  BEGIN
                     ...
                     BEGIN
                        ...
                        PRINT '@DIASLABT='+STR(@DIASLABT)
                        PRINT '@BASICO='+STR(@BASICO)
                        PRINT  'NUEVA FORMULA'+STR( (((@BASICOT/30)*@DIASINDEMAYOR)*@DIASLABT)/360)
                        PRINT '@DIASLABT/360.00 ='+STR(@DIASLABT/360)
                        PRINT '@DIASLABT%360.00 ='+STR(@DIASLABT%360)
                        PRINT '@VALORD ANTES PRIMER ANO ='+STR(@VALORD)
                        SELECT @VALORD=@VALORD+(((@BASICOT/30)*@DIASINDEMAYOR)*@DIASLABT)/360
                        
                        --SELECT @VALORD=@VALORD+((@DIASLABT/360)*(@BASICO*@DIASINDEMAYOR))
                        --PRINT '@VALORD ='+STR(@VALORD)
                        --SELECT @RESIDUO=(@DIASLABT%360)
                        --PRINT  '@RESIDUO='+STR(@RESIDUO)
                        --SELECT @POR_RESIDIO=ROUND(((@RESIDUO*@DIASINDEMAYOR)/360.00),1)
                        --PRINT '@POR_RESIDIO ='+CAST(@POR_RESIDIO AS VARCHAR(20))
                        --PRINT '(@BASICO*@POR_RESIDIO) ='+CAST((@BASICO*@POR_RESIDIO) AS VARCHAR(20))
                        --SELECT @VALORD=@VALORD+(@BASICO*@POR_RESIDIO)
                        PRINT '@VALORD FINAL ='+STR(@VALORD)
                     END
                  END
               END
            END
