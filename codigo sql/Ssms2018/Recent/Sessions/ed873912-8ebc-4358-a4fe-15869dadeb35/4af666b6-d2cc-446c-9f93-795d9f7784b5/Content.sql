...
BEGIN
   ...
   */ --Se conoce que los partoculares SI SE REPORTAN STORRES_20250403
	...
	BEGIN
		BEGIN --AC
            ...
            CASE WHEN CIT.IDPLAN IN (SELECT DATO FROM USVGS WHERE IDVARIABLE IN ('IDPLANPART','IDPLANPART1', 'IDPLANPART2', 'IDPLANPART3', 'IDPLANPART4', 'IDPLANPART5'))
                 ...
                 ELSE COALESCE(CIT.VALORTOTAL,0)
		...
		BEGIN --AP
			...
			SELECT @IDPRESTADOR,REPLACE(CONVERT(VARCHAR,CIT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,CIT.FECHA,108),5),null,
					...
					CIT.IDDX,COALESCE(CIT.IDDX,'null'),COALESCE(CIT.VALORTOTAL,0)*COALESCE(CIT.CANTIDADC,1),'04',COALESCE(CIT.VALORCOPAGO,0),@N_FACTURA
			FROM CIT 
				INNER JOIN SER ON CIT.IDSERVICIO=SER.IDSERVICIO
				INNER JOIN RIPS_CP ON SER.CODIGORIPS=RIPS_CP.IDCONCEPTORIPS
			   INNER JOIN MED ON CIT.IDMEDICO=MED.IDMEDICO
	...
	BEGIN
		BEGIN --AC
			...
			SELECT @IDPRESTADOR,fechaInicioAtencion=REPLACE(CONVERT(VARCHAR,AUT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,AUT.FECHA,108),5),
				...
				COALESCE(MED.TIPO_ID, @TIPODOC,''), COALESCE(MED.IDMEDICO, @DOCIDAFILIADO,'') --STORRES_20250326
	...
	IF @CANT>0 
	BEGIN
      PRINT 'ANTES DE CONSULTAS '+COALESCE(@PLANO,'SIN PLANO')
		...
		BEGIN
			...
			SET @PLANO+= '"conceptoRecaudo":"'+ CASE WHEN TRY_CAST(@valorPagoModerador AS decimal(14,2))<=0 THEN '05' WHEN (@TIPOUSU IN ('01', '02', '04') AND @conceptoRecaudo = '01') THEN '03'  ELSE @conceptoRecaudo /* IIF(@conceptoRecaudo='01','03',@conceptoRecaudo)*/ END +'",'--YCARRILLO
			SET @PLANO+='"valorPagoModerador":'+@valorPagoModerador+','
			--SET @PLANO+='"numFEVPagoModerador": "'+@N_FACTURA+'",'
         SET @PLANO+='"numFEVPagoModerador": "'+ IIF(@conceptoRecaudo = '05','null',@N_FACTURA)+'",'
			SET @PLANO+='"consecutivo": '+@consecutivo+''
			SET @PLANO+='}'
			SELECT @NRO+=1
		END		
		SET @PLANO  += ']'
      PRINT 'DESPUIES DE CONSULTAS '+COALESCE(@PLANO,'SIN PLANO')
	END	
	SELECT @CANT=COUNT(1) FROM @MEDICAMENTOS
	IF @CANT>0
	BEGIN
   PRINT 'ANTES DE MEDICAMNENTOS '+COALESCE(@PLANO,'SIN PLANO')
		...
		BEGIN
         ...
         SET @PLANO+='"numFEVPagoModerador": "'+ IIF(@conceptoRecaudo = '05','null',@numFEVPagoModerador)+'",'
			SET @PLANO+= '"consecutivo":'+@consecutivo+''
			SET @PLANO+='},'
			SELECT @NRO+=1
		END		
      ...
      PRINT 'DESPUES DE MEDICMANETO '+COALESCE(@PLANO,'SIN PLANO')
	END
	
	SELECT @CANT=COUNT(1) FROM @PROCEDIMIENTOS
	IF @CANT>0
	BEGIN
      PRINT 'ANTES DE PROCEDEMIENTOS '+COALESCE(@PLANO,'SIN PLANO')
		IF @primerGrupo = 1 SELECT @primerGrupo = 0
		ELSE SET @PLANO += ','
		...
		SET @PLANO  += ']'
      PRINT 'DESPUES DE PROCEDEMIENTOS '+COALESCE(@PLANO,'SIN PLANO')
	END
--query2
--query2
	...
	IF @CANT>0
	BEGIN
      PRINT 'ANTES DE OTROS '+COALESCE(@PLANO,'SIN PLANO')
      SELECT * FROM @OTROSSER
		...
		END	
		SET @PLANO += ']'
      PRINT 'DESPUES DE OTROS '+COALESCE(@PLANO,'SIN PLANO')
	END
	...
	END
      ...
      --SELECT '@CONSULTAS' AS DATO, * FROM @CONSULTAS
      --SELECT '@MEDICAMENTOS' AS DATO, * FROM @MEDICAMENTOS
      --SELECT '@PROCEDIMIENTOS' AS DATO, * FROM @PROCEDIMIENTOS
      --SELECT '@OTROSSER' AS DATO, * FROM @OTROSSER
