...
BEGIN
   ...
   SELECT  CXP.CODNOMINA,CXP.CODPRF,CXP.DESCRIPCION,CXP.NUMDOC,NEMP.TERCEROID,CXP.FECHA,SUM(CXP.VALOR_NETO),SUM(SINDEDUC)
   FROM(
   ...
   )CXP INNER JOIN NEMP ON CXP.NUMDOC=NEMP.NUMDOC
   GROUP BY CXP.CODNOMINA,CXP.CODPRF,CXP.DESCRIPCION,CXP.NUMDOC,NEMP.TERCEROID,CXP.FECHA
   ORDER BY CXP.CODPRF DESC,CXP.NUMDOC ASC

   OPEN GENCXP
   
   FETCH NEXT FROM GENCXP
   INTO @CODNOMINA, @CODPRF_CUR, @DESCRIPCION_CUR, @IDTERCERO_CUR,@TERCEROID, @FECHA_CUR, @VALOR_CUR,@VLR_SINDTO
   ...
   BEGIN
     ...
     WHERE NIEPE.CNSPAGO=@CNSPAGO AND NIEPED.CODPRF=@CODPRF_CUR AND NIEPED.IDTERCERO_PRF=@TERCEROID
	 ...
	 BEGIN 
		  ...
		  WHERE PROCEDENCIA='NOMINA' AND REFERENCIA2=@CNSPAGO AND  IDTERCERO=@TERCEROID  AND ESTADO=2
		  AND COALESCE(ANULADO,0)=0
			...
			IF(COALESCE(@NROCOMPROBANTE,'')='')
			 BEGIN 
			  ...
			  WHERE MCP.PROCEDENCIA='NOMINA' AND MCP.REFERENCIA2=@CNSPAGO AND  MCH.IDTERCERO=@TERCEROID  AND MCP.ESTADO=2 
			  AND COALESCE(ANULADO,0)=0
			 END 
	 END
     ...
     INSERT INTO FCXP (CNSFCXP,FECHA,CNSFCOM,IDTERCERO,NOREFERENCIA,F_FACTURAREF,F_VENCE,VALOR,ESTADO,F_CANCELADO,USUARIO,
      ...
      SELECT @CNSFCXP, DBO.FNK_FECHA_SIN_MLS(GETDATE()),@CODNOMINA,@TERCEROID, @CNSPAGO, @FECHA_CUR, 
     ...
     END,CCOSTO,IDAREA,CUENTA,FD.CNSPAGO,LINEA,CODCONCEPTO,DESCRIPCION,RUBRO,RUBRO_ID,SINDEDUC
      ...
      AND    C.CLASE      = 'Salario' 
      AND    A.IDTERCERO  = @TERCEROID 
      AND    A.NUMDOC=@IDTERCERO_CUR
    ...
    BEGIN
      ...
      BEGIN
         ...
         SELECT @PLANTILLA=IDPLANTILLA FROM NOMI WHERE CODNOMINA=@CODNOMINA
         ...
         AND NPLTCO.IDPLANTILLA=@PLANTILLA
    ...
    BEGIN
       ...
       BEGIN
          ...
          SELECT @CCOSTOPPTO=DBO.FNK_VALORVARIABLE('PRESUP_CCOSTO')
	        ...
	        IF (SELECT ESTADO FROM  PTRP WHERE CONSECUTIVO=@PPPTO AND COMPANIA='01' AND CDP=@CDP AND IDTERCERO=@TERCEROID AND RP=@CNSRP)='NoConfirmado'
	        BEGIN
