...
BEGIN
   ...
   BEGIN
      ...
      SELECT @sUrl=LTRIM(RTRIM(@sUrl))+'CreatePaymentOrder'
	   SELECT @HOY =   DATEADD(HOUR,24,GETDATE()), @DIA =   convert(varchar, @HOY, 3),@MES =   convert(varchar, @HOY, 1), @ANIO =  convert(varchar, @HOY, 23), @HORA =  convert(varchar, @HOY, 8)
      ...
      INSERT INTO KPAGE(CONSECUTIVO, TABLA, PROCEDENCIA, FECHAGEN,MODO, VALOR, ESTADO,USUCOBRA, IDPLAN,REF_PAGO,RESPONSABLEPAGO,F_EXPIRA)
   	SELECT @NOADMISION,@TABLA,@PROCEDENCIA, CONVERT(SMALLDATETIME,GETDATE()),@MODO, @VALOR, 'CREATED',@USUARIO,NULL,@N_FACTURA,@IDAFILIADO,@HOY
      ...
      SELECT @BODY=@BODY+' "expirationDate": "'+@DATE+'+05:00",'
   ...
   ELSE
   BEGIN
      UPDATE KPAGE SET ESTADO='ERROR' WHERE IDKPAGE = @IDKPAGE	
   END
END
