...
DECLARE  @PARAMETROS NVARCHAR(MAX)			,@MODELO VARCHAR(100)			   ,@METODO VARCHAR(100)
		...
		,@CUOTAS SMALLINT               ,@NROCOMPROBANTE VARCHAR(20)  ,@DIASDIF SMALLINT             ,@TIPOFIN VARCHAR(1)
BEGIN
	SELECT *
	INTO #JSON
	...
	BEGIN
		...
		SELECT @CNSFTR=CNSFTR,@N_FACTURA=N_FACTURA,@IDSEDEF=IDSEDEF,	@F_FACTURA=F_FACTURA,@F_VENCE=F_VENCE,	@IDTERCEROF=IDTERCERO,	@IDPLAN=IDPLAN,	
		...
		BEGIN
			...
			BEGIN TRY
				...
				INSERT INTO FTR(CNSFCT, COMPANIA, CLASE, IDTERCERO, N_FACTURA, F_FACTURA, F_VENCE, VR_TOTAL, COBRADOR, VENDEDOR, MONEDA, OCOMPRA, ESTADO, F_CANCELADO,
								...
								IDSEDE, RDIAN, CNSRESOL,IDFORMATO,COPAPROPIO,CP_VLR_COPAGOS,CAPITADA,CP_CONVENIO,CP_MODALIDAD,CP_MES,FECHACAP_INI,FECHACAP_FIN, VALORMODERADORA,SINCNT,CUOTAS)
				SELECT @CNSFTR, @COMPANIA, 'C', @IDTERCEROF, @NVOCONSEC, CASE  WHEN  @F_FACTURA IS NOT NULL THEN @F_FACTURA ELSE DBO.FNK_GETDATE() END,
					...
					,0, NULL, NULL, LEFT(@DATO,2), NULL, 'P', NULL, 
				...
				@VLRCOPAPRO,@CAPIPGP,@CP_CONVENIO,@CP_MODALIDAD,@CP_MES,@FECHACAP_INI,@FECHCACA_FIN, @VALORMODERADORA,@SINCNT,COALESCE(@CUOTAS,0)
		...
		SELECT FTR.N_FACTURA,FTR.IDTERCERO,TER.NIT, TER.RAZONSOCIAL,FTR.F_FACTURA,FTR.F_VENCE,FTR.PROCEDENCIA,FTR.NOREFERENCIA,FTR.IDPLAN,FTR.VALORSERVICIOS,COALESCE(FTR.VALORCOPAGO,0)VALORCOPAGO,
				...
				FECHACAP_FIN AS F_FINAL,IDSEDE,CP_MES AS MES,CP_MODALIDAD AS MODALIDAD,OBSERVACION,TIPOFIN AS TIPOFINAN,PLN.DESCPLAN,USUARIOFACTURA,CP_VLR_SERVICIOS,CP_VLR_COPAGOS, FTR.VALORCOPAGO , FTR.VALORMODERADORA,FTR.SINCNT
				,COALESCE(FACTEP,'Sin Notificar') FACTEP
         ...
         END
			...
			BEGIN TRY 
				INSERT INTO FTRD(CNSFTR,N_CUOTA,AREAPRESTACION,PREFIJO,REFERENCIA,ANEXO,CANTIDAD,VALOR,VR_TOTAL,VLR_SERVICI,
									IDPLAN,CCOSTO,N_FACTURA,FECHA,COPAGOS_CP,VIVA,PIVA)
				SELECT @CNSFTR,@N_CUOTA,@IDAREA,@PREFIJO,@IDSERVICIO,@ANEXO,@CANTIDAD,
					...
					@VR_TOTAL,@VR_TOTAL,@IDPLAN,@CCOSTO,@N_FACTURA,dbo.FNK_GETDATE(),COALESCE(@VLR_CPROPIOFRD,0),
					CASE WHEN @IXCOUNTRY='PERU' THEN @VALOR-ROUND(@VALOR/1.18,2)  ELSE  NULL  END ,
					CASE WHEN @IXCOUNTRY='PERU' THEN 18 ELSE NULL  END 
			END TRY 
			BEGIN CATCH
		...
		BEGIN
			BEGIN TRY
				...
				UPDATE FTRD SET
					...
					CCOSTO=@CCOSTO,
					VIVA=CASE WHEN @IXCOUNTRY='PERU' THEN @VALOR-(@VALOR/1.18)  ELSE  NULL  END ,
					PIVA=CASE WHEN @IXCOUNTRY='PERU' THEN 18 ELSE NULL  END 
				WHERE CNSFTR=@CNSFTR
					AND N_CUOTA=@N_CUOTA
...
--QUERY 3
	...
	BEGIN 
		...
		WHERE CNSFTR = @CNSFTR AND N_CUOTA=@N_CUOTA
	...
	BEGIN         
		...
		ELSE
		BEGIN
			INSERT INTO @TBLERRORES SELECT 'No se Encontro el Item de la Cotización'
		END
		IF(SELECT COUNT(1) FROM @TBLERRORES)>0
		BEGIN
			SELECT 'KO' OK, ERROR FROM @TBLERRORES
	...
	BEGIN         
		...
		BEGIN
			INSERT INTO @TBLERRORES(ERROR)
			SELECT 'No se Encontro cotización Activa o no Tiene detalles, verifique e intente de nuevo'
		END
		IF(SELECT COUNT(1) FROM @TBLERRORES)>0
		BEGIN
			SELECT 'KO' OK, ERROR FROM @TBLERRORES
