...
--- VARIABLES PARA PRESTAMOS    
	...
	SET @NUMCUOTA_CUR        = 0    
	SET @CUOTASPGR           = 0    
	SET @CONCEPTO_CON_EVENTO = (SELECT COUNT(1) FROM NEVEC WHERE IDCONCEPTO=@CODCONCEPTO)
...
--  SET @HORAS_A_LABORAR     = @HORASXCARGO * (@DIASXPERIODO/@DIASXCARGO)  
	...
	BEGIN 
		...
		IF @HORAS_SUMARESTA IS NULL   
			...
			BEGIN
				SELECT @HORAS_SUMARESTA=COALESCE(@HORAS_SUMARESTA,0)+CASE WHEN DAY(FECHAFIN)<>DAY(@FECHAFIN) THEN (((DATEDIFF(DAY,FECHAFIN,@FECHAFIN) + CASE WHEN DAY(@FECHAFIN) <@DIAS_NPPAG THEN @DIAS_NPPAG-DAY(@FECHAFIN) ELSE 0 END) *@HORADIA)*-1) ELSE 0 END
