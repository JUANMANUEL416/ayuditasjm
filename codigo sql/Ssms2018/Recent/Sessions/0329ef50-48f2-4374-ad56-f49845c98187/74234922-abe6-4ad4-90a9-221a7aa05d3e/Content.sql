...
BEGIN 
  ...
  -- END
   ...
   FROM   (
            SELECT DISTINCT @ENVIO AS ENVIO, LEFT(AFI.TIPO_DOC,2) AS TIPO_DOC, AFI.DOCIDAFILIADO, LEFT(TER.IDALTERNA2,6) AS IDALTERNA2, 
  ...
  CASE WHEN @RIPS_FEHAINIFINAL='SI' THEN DBO.FNK_FECHA_SIN_HORA(HADM.FECHA) ELSE @FECHAINICIAL END F_INI,  
  CASE WHEN @RIPS_FEHAINIFINAL='SI' THEN DBO.FNK_FECHA_SIN_HORA(COALESCE(HADM.FECHAALTAMED,@FECHAFINAL)) ELSE @FECHAFINAL END  F_FIN, 
          LEFT(TER.IDALTERNA2,6), LEFT(TER.RAZONSOCIAL,30), LEFT(COALESCE (CNT.IDALTERNA, PPT.NROCONTRATO, CASE COALESCE(@RIPS_IDCONTRATO,'') when 'NO' THEN '' ELSE HADM.IDPLAN END ),15)CONTRATO, RIGHT(HADM.NOPOLIZA,10), 
 ...
 FROM   RIPSD INNER JOIN FTRD   ON RIPSD.N_FACTURA     = FTRD.N_FACTURA
  ...
  CASE WHEN @RIPS_FEHAINIFINAL='SI' THEN DBO.FNK_FECHA_SIN_HORA(HADM.FECHA) ELSE @FECHAINICIAL END F_INI,  
  CASE WHEN @RIPS_FEHAINIFINAL='SI' THEN DBO.FNK_FECHA_SIN_HORA(COALESCE(HADM.FECHAALTAMED,@FECHAFINAL)) ELSE @FECHAFINAL END  F_FIN, 
          LEFT(TER.IDALTERNA2,6), LEFT(TER.RAZONSOCIAL,30), 
          LEFT(COALESCE (CNT.IDALTERNA, PPT.NROCONTRATO, CASE COALESCE(@RIPS_IDCONTRATO,'') when 'NO' THEN '' ELSE HADM.IDPLAN END ),15)CONTRATO, RIGHT(HADM.NOPOLIZA,10), 
 CASE WHEN @RIPS_COPAGOS='NO' THEN CASE WHEN @RIPS_COPAGOS_FTR='SI' THEN  CAST (FTR.VALORCOPAGO AS INT ) ELSE  0 END  ELSE CAST (FTR.VALORCOPAGO AS INT ) END VALORCOPAGO , '0', '0', 
 CASE WHEN @RIPS_COPAGOS='NO' THEN CASE WHEN @RIPS_COPAGOS_FTR='SI' 
                                                                  THEN  CONVERT (VARCHAR (15),CONVERT(DECIMAL(14,2), FTR.VR_TOTAL)) 
                                                                  ELSE CONVERT (VARCHAR (15), CAST (FTR.VALORCOPAGO AS INT )+ CONVERT(DECIMAL(14,2), FTR.VR_TOTAL))  
 ...
 ELSE  CONVERT (VARCHAR (15),CONVERT(DECIMAL(14,2), FTR.VR_TOTAL)) END
  ...
  BEGIN --COVID
     ...
     BEGIN
		...
		BEGIN
        UPDATE RRIPS_AF SET FECHAINICIO= @FECHAINICIAL, FECHAFINAL=@FECHAFINAL
        WHERE ENVIO=@ENVIO AND IDTERCERO=@IDTERCERO AND NUMFACTURA=@N_FACTURA
		
        UPDATE RRIPS_AU SET FECHAINGRESO=@FECHAINICIAL, FECHASALIDA=@FECHAFINAL
        WHERE ENVIO=@ENVIO AND IDTERCERO=@IDTERCERO AND NUMFACTURA=@N_FACTURA

        UPDATE RRIPS_AH SET FECHAINGRESO=@FECHAINICIAL, FECHAEGRESO=@FECHAFINAL
        WHERE ENVIO=@ENVIO AND IDTERCERO=@IDTERCERO AND NUMFACTURA=@N_FACTURA
		END
        SET @I+=1
     END
