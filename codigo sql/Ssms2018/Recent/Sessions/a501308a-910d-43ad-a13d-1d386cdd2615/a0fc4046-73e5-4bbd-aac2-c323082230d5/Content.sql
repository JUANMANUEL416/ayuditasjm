...
DECLARE @DIASCAL	 INT  
	...
	SET @NUMCUOTA_CUR        = 0    
	SET @CUOTASPGR           = 0    
	SET @CONCEPTO_CON_EVENTO = (SELECT COUNT(1) FROM NEVEC WHERE IDCONCEPTO=@CODCONCEPTO AND COALESCE(ENLIQUI,0)=1)
	IF ISNULL(@CONCEPTO_CON_EVENTO,0)=0
		SET @CONCEPTO_CON_EVENTO = (SELECT COUNT(1) FROM NEVE WHERE CODCONCEPTO=@CODCONCEPTO AND COALESCE(ENLIQUI,0)=1)
