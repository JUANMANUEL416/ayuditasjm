IF EXISTS (SELECT NAME FROM SYSOBJECTS WHERE NAME = 'SPK_GENERA_JSON_PERU_FACTURA' AND TYPE = 'P')
BEGIN
   DROP PROCEDURE SPK_GENERA_JSON_PERU_FACTURA
END
GO
CREATE PROCEDURE DBO.SPK_GENERA_JSON_PERU_FACTURA 
      ...
      ORDER BY N_CUOTA ASC
	   ...
	   BEGIN 
         SELECT @ITEM=CONVERT(varchar(3),N_CUOTA),@CANTIDAD=CONVERT(varchar(4),CONVERT(INT,CEILING(FTRD.CANTIDAD))),
                ...
                @VALOR2=CONVERT(VARCHAR(20),CONVERT(DECIMAL(14,4),COALESCE(VALOR,0)+CASE WHEN COALESCE(PIVA,0)>0 THEN VALOR*(PIVA/100) ELSE 0 END)),
                @VALOR3=CONVERT(VARCHAR(20),CONVERT(DECIMAL(14,2),ROUND((COALESCE(VALOR,0)*COALESCE(FTRD.CANTIDAD,0)- COALESCE(FTRD.VLR_COPAGOS,0)),2))),
                ...
                @VALOR6=CASE WHEN COALESCE(VIVA,0)>0 THEN  CONVERT(VARCHAR(20),CONVERT(DECIMAL(14,2),COALESCE(VIVA,0),0)) ELSE '0.00' END,
  ...
  SELECT @BODY=@JSON
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
   IF COALESCE(@FACTEP,'')='ERROR' OR( COALESCE(@FACTEP,'')=''  AND @ERFACTEP NOT LIKE '%no existe%' AND @ERFACTEP NOT LIKE '%se encontro errores%')
   BEGIN
      PRINT 'INGRESO A CONSULTAR LOS DATOS EN MI FACT'
      EXEC DBO.SPK_CONSULTA_DOCUMENTO_ELECTRONICA @N_FACTURA

      IF EXISTS(SELECT * FROM FTR WHERE N_FACTURA=@N_FACTURA AND FACTEP='OK')
      BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @FACTEP='OK'
         UPDATE FTR SET FACTEP=@FACTEP,LINKFACTURA=@URLFACTEP,RAZONANULACION='' WHERE N_FACTURA=@N_FACTURA
   ...
   ELSE
   BEGIN
      UPDATE FTR SET FACTEP='ERROR',RAZONANULACION='Factura no Recibida en MiFact, Error de estructura' WHERE N_FACTURA=@N_FACTURA
   END
END
