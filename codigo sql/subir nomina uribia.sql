...
SELECT DISTINCT DOCUMENTO INTO #X FROM #H WHERE NOT EXISTS(SELECT * FROM NEMPFD WHERE CNSPAGO='02202501P30-1' AND NEMPFD.NUMDOC=#H.DOCUMENTO) AND #H.VALOR>0 AND NOMI='ope'

SELECT  * FROM NEMPF INNER JOIN NEMP ON NEMPF.NUMDOC=NEMP.NUMDOC
WHERE NOT EXISTS(SELECT * FROM #H WHERE NEMP.DOCIDEMPLEADO=#H.DOCUMENTO AND #H.NOMI='OPE')
