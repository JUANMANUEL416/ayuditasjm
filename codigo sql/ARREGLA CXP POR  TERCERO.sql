IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE NAME='SPK_GENCXP_NOMINA_TERCERO_X' AND XTYPE='P')
BEGIN
   DROP PROCEDURE SPK_GENCXP_NOMINA_TERCERO_X
END
GO
CREATE PROCEDURE DBO.SPK_GENCXP_NOMINA_TERCERO_X 
@CNSPAGO          VARCHAR(20),
@COMPANIA         VARCHAR(2),
@SEDE             VARCHAR(5),
@USUARIO          VARCHAR(12),
...
@TERCERO          VARCHAR(20),
@CODPRF       VARCHAR(20)=NULL,
@CNSFCXP       VARCHAR(20)=NULL
--WITH ENCRYPTION
AS
...
BEGIN
   ...
   PRINT 'Eliminando CXP generadas anteriormente para este Pago. @CNSFCXP=  '+COALESCE(@CNSFCXP,'NO TRAIGO LA CXP')
   DELETE FCXPD 
   FROM   FCXPD INNER JOIN FCXP  ON FCXPD.CNSFCXP = FCXP.CNSFCXP 
   WHERE  FCXP.PROCEDENCIA  = 'Nomina' 
   AND    FCXP.NOREFERENCIA = @CNSPAGO 
   AND    FCXP.IDTERCERO    = @TERCERO
   AND   FCXP.CNSFCXP=CASE WHEN COALESCE(@CNSFCXP,'')<>'' THEN @CNSFCXP ELSE FCXP.CNSFCXP END

   IF COALESCE(@CNSFCXP,'')=''
   BEGIN
	   SELECT @CNSFCXP=CNSFCXP FROM FCXP
	   WHERE  FCXP.PROCEDENCIA  = 'Nomina' 
	   AND    FCXP.NOREFERENCIA = @CNSPAGO 
	   AND    FCXP.IDTERCERO    = @TERCERO
  END
   ...
   SELECT  CXP.CODNOMINA,CXP.CODPRF,CXP.DESCRIPCION,CXP.NUMDOC,CXP.FECHA,SUM(CXP.VALOR_NETO)
   FROM(
       SELECT B.CODNOMINA, C.CODPRF, C.DESCRIPCION, A.NUMDOC, DBO.FNK_FECHA_SIN_HORA(B.FECHAFIN) FECHA, 
              A.VALOR_NETO
       FROM   NEMPF A INNER JOIN NPER B ON A.CNSPAGO = B.CNSPAGO, NPRF C
       WHERE  A.CNSPAGO    = @CNSPAGO
       AND    A.NUMDOC=    @TERCERO
       AND    C.CLASE      = 'Salario' 
       AND    A.VALOR_NETO > 0 
       ...
       WHERE  A.CNSPAGO = @CNSPAGO AND D.GENERA_CXP = 'Si' AND C.CLASE <> 'Salario'
        ...
        -- AND F.TIPO=CASE WHEN F.CLASE='Normal' AND EXISTS(SELECT * FROM NEMP WHERE NUMDOC=A.IDTERCERO) THEN 'Ingreso' ELSE F.TIPO END
       GROUP BY B.CODNOMINA, C.CODPRF, C.DESCRIPCION, A.IDTERCERO, A.CNSPAGO, DBO.FNK_FECHA_SIN_HORA(B.FECHAFIN), F.TIPO,A.CODCONCEPTO
   )CXP
   WHERE CXP.CODPRF=CASE WHEN COALESCE(@CODPRF,'')<>'' THEN @CODPRF ELSE CXP.CODPRF END
   ...
   --      AND NIEPE.NUMDOC=@IDTERCERO_CUR
     ...
     END
      ...
      SELECT @PORCENTAJE=(CONVERT(DECIMAL(14,6),(VALOR_EGRESOS*100/VALOR_INGRESOS))/100) FROM NIEPE WHERE CNSPAGO=@CNSPAGO AND NUMDOC=@IDTERCERO_CUR
      DECLARE CUR2 CURSOR FOR 

     SELECT NUMDOC,CANTIDAD,CASE WHEN EXISTS(SELECT * FROM NEMPE WHERE NUMDOC=@IDTERCERO_CUR) 
     AND EXISTS(SELECT * FROM NCON X WHERE X.CODCONCEPTO=CODCONCEPTO AND X.CLASE='Ingeso') THEN
         CONVERT(DECIMAL(14,2),VALOR-ROUND((VALOR*COALESCE(@PORCENTAJE,0)),2))
     ELSE
         CONVERT(DECIMAL(14,2),VALOR)
     END,
      CCOSTO,IDAREA,CUENTA,FD.CNSPAGO,LINEA,CODCONCEPTO,DESCRIPCION,RUBRO
      FROM (
      SELECT A.IDTERCERO,A.NUMDOC, CANTIDAD = 1, A.VALOR,H.VALOR_EGRESOS,
             ...
             A.CNSPAGO, A.LINEA, A.CODCONCEPTO, G.DESCRIPCION,D.RUBRO,C.CLASE,G.TIPO
      FROM   NEMPFD A INNER JOIN NPER   B ON A.CNSPAGO     = B.CNSPAGO 
