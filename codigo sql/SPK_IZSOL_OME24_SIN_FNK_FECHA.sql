IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE NAME='SPK_IZSOL_OME24_SIN_FNK_FECHA' AND XTYPE='P')
BEGIN
   DROP PROCEDURE SPK_IZSOL_OME24_SIN_FNK_FECHA
END
GO
CREATE PROC DBO.SPK_IZSOL_OME24_SIN_FNK_FECHA
...
DECLARE @NOADMISION     VARCHAR(16),     @VECES           INT,           @HABCAMA          VARCHAR(10),    @SECTOR           VARCHAR(10),
        ...
        @CONFAUT        SMALLINT,        @TIPOBODEGA VARCHAR(10) 
BEGIN
   ...
   BEGIN
      ...
      BEGIN 
         ...
         BEGIN
            ...
            BEGIN
               IF (SELECT COUNT(*) FROM HBALI WHERE NOADMISION = @NOADMISION AND CONVERT(VARCHAR,FECHA,104) = CONVERT(DATETIME,CONVERT(VARCHAR,@FECHAULT-1,104))) = 0
               BEGIN
                  ...
                  SELECT @CNSHBALI, @NOADMISION, CAST(CONVERT(VARCHAR,@FECHAULT-1,104) AS DATETIME)
               END
               ELSE
               BEGIN
                  SELECT @CNSHBALI = CNSHBALI FROM HBALI WHERE NOADMISION = @NOADMISION AND CONVERT(VARCHAR,FECHA,104) = CONVERT(DATETIME,CONVERT(VARCHAR,@FECHAULT-1,104))
               END
            ...
            BEGIN   --'DESPUES DE 7'
               ...
               IF (SELECT COUNT(*) FROM HBALI WHERE NOADMISION = @NOADMISION AND CONVERT(VARCHAR,FECHA,104) = CONVERT(DATETIME,CONVERT(VARCHAR,@FECHAULT,103)))= 0
               BEGIN                        
                  EXEC SPK_GENCONSECUTIVO '01',@IDSEDE,'@HBALI', @CNSHBALI OUTPUT  
                  SELECT @CNSHBALI = @IDSEDE + REPLACE(SPACE(10 - LEN(@CNSHBALI))+LTRIM(RTRIM(@CNSHBALI)),SPACE(1),0) 
                  INSERT INTO HBALI(CNSHBALI, NOADMISION, FECHA)
                  SELECT @CNSHBALI, @NOADMISION, CAST(CONVERT(VARCHAR,@FECHAULT,104)AS DATETIME)
               END
               ELSE
               BEGIN
                  SELECT @CNSHBALI = CNSHBALI FROM HBALI WHERE NOADMISION = @NOADMISION AND CONVERT(VARCHAR,FECHA,104) = CONVERT(DATETIME,CONVERT(VARCHAR,@FECHAULT,104))
               END
               IF (SELECT COUNT(*) FROM HBALID WHERE CNSHBALI = @CNSHBALI AND IDSERVICIO = @IDSERVICIO AND CLASE = @CODOM) =  0
               BEGIN
...
--query 2
      ...
      SELECT @CONFAUT = COALESCE(CONFIRMAAUTOMDESDEQX,0),@TIPOBODEGA=TIPOBODEGA FROM IBOD WHERE IDBODEGA = @IDBODEGAATIENDE
