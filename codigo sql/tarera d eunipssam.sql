...
SELECT @FECHAINI='28/03/2025' --DBO.FNK_FECHA_SIN_MLS(GETDATE())
SELECT @FECHAMAX=@FECHAINI+1


	DECLARE PG_CURSOR CURSOR FOR   
   SELECT  NOADMISION FROM HADM WHERE COALESCE(FECHAALTAMED,GETDATE())>=@FECHAINI AND FECHA<@FECHAMAX AND FACTURABLE=1 
	OPEN PG_CURSOR    
