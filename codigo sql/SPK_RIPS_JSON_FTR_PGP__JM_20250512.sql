...
BEGIN
   ...
   WHERE  EXISTS(SELECT * FROM FTRDC WHERE  FTRDC.CNSFTR=@CNSFCT AND FTRDC.NOADMISION=HADM.NOADMISION AND FTRDC.PROCEDENCIA='HADM')
	...
	BEGIN 
			...
			numAutorizacion=LEFT(COALESCE(CIT.NOAUTORIZACION,'null'),30),codConsulta=LEFT(REPLACE(REPLACE(LTRIM(RTRIM(SER.CODCUPS)),CHAR(13),''),CHAR(10),''),6),modalidadGrupoServicioTecSal='01',grupoServicios='01',
			...
			causaMotivoAtencion='38',CIT.VALORTOTAL,IIF(COALESCE(FTRDC.VLR_COPAGO,0)>0,IIF(COALESCE(AFI.TIPOUSUARIO,'')IN('01','02','03'),'02',IIF(COALESCE(AFI.TIPOUSUARIO,'')='04','01',@conceptoRecaudo)),'05'),COALESCE(FTRDC.VLR_COPAGO,0), MED.TIPO_ID, MED.IDMEDICO,
         NULL,NULL
	...
	BEGIN
		...
		causaMotivoAtencion='38',AUTD.VALOR,IIF(COALESCE(AUTD.VALORCOPAGO,0)>0,'02','05'),COALESCE(FTRDC.VLR_COPAGO,0), COALESCE(MED.TIPO_ID, @TIPODOC,''), COALESCE(MED.IDMEDICO, @DOCIDAFILIADO,''),
	...
	BEGIN 
		...
		SELECT @IDPRESTADOR,CIT.IDAFILIADO,CIT.CONSECUTIVO,REPLACE(CONVERT(VARCHAR,CIT.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,CIT.FECHA,108),5),null,
				...
				CASE WHEN COALESCE(CIT.IDDX,'')='' THEN 'null' END,CASE WHEN COALESCE(CIT.IDDX,'')='' THEN 'null' END,CIT.VALORTOTAL,IIF(COALESCE(FTRDC.VLR_COPAGO,0)>0,'02','05'),'04',COALESCE(FTRDC.VLR_COPAGO,0),@N_FACTURA
		FROM  FTRDC 
		...
		BEGIN --AP
			...
			SELECT @IDPRESTADOR,AUT.IDAFILIADO,AUT.NOAUT,REPLACE(CONVERT(VARCHAR, AUT.FECHA, 102), '.', '-') + ' ' + LEFT(CONVERT(VARCHAR, AUT.FECHA, 108), 5),NULL
				...
				,AUT.DXPPAL,IIF(COALESCE(AUT.DXRELACIONADO,'')='',AUT.DXPPAL,AUT.DXRELACIONADO),AUT.DXPPAL,AUTD.VALOR,IIF(COALESCE(AUT.VALORCOPAGO,0)>0,'02','05'),FTRDC.CANTIDAD,'04'
   ...
   END
	...
	BEGIN 
      ...
      DECLARE @F_INIPERIODO DATETIME
		...
		CASE WHEN HADM.FECHA < @F_INIPERIODO THEN REPLACE(CONVERT(VARCHAR,@F_INIPERIODO,102),'.','-')+' 00:01'  ELSE  REPLACE(CONVERT(VARCHAR,HADM.FECHA,102),'.','-')+' '+LEFT(CONVERT(VARCHAR,HADM.FECHA,108),5)END
			,HADM.NOAUTORIZACION
