    IF EXISTS(SELECT NAME FROM SYSOBJECTS WHERE NAME= 'SPK_SOL_REEMBOLSO_CAJAMENOR_ACJ' AND TYPE='P')
BEGIN
   DROP PROCEDURE SPK_SOL_REEMBOLSO_CAJAMENOR_ACJ
END
GO
CREATE PROC DBO.SPK_SOL_REEMBOLSO_CAJAMENOR_ACJ 
@CODCAJA          VARCHAR(4),
...
BEGIN  
  ...
  -- EXEC DBO.SPK_CIERRE_DE_CAJA @CODCAJA, @SYS_COMPUTERNAME, @SEDE, @COMPANIA
   ...
   IF  EXISTS(SELECT * FROM  FCJ  WHERE   FCJ.CODCAJA = @CODCAJA 
				AND    (FCJ.CONTABILIZADA = 0 OR FCJ.CONTABILIZADA IS NULL)
			    AND    FCJ.CNSACJ=@CNSACJ AND COALESCE(CNSCMREE,'')='')
   BEGIN 
    ...
    ELSE
	BEGIN
		SELECT @CNSCMREE= CNSCMREE FROM CMREE WHERE CODCAJA= @CODCAJA AND (FECHA>= @FECHA OR FECHA<@FECHA+1) AND ESTADO='S'
	END
   ...
   SELECT @NVOCONSEC, @FECHA, NULL, @RESPONSABLE, @N_FACTURA /*@CODCAJA*/, @FECHA, @FECHA, 0, 'N', NULL, @USUARIO, 
          ...
          YEAR(@FECHA), MONTH(@FECHA), NULL, NULL, @CIUDAD,0
